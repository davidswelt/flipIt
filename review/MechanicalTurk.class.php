...
* Modified and extended by Alan Nochenson <anochenson@gmail.com>
*/
...
class MechanicalTurk {
	...
	private $MTURK_ROOT_URL = 'https://mechanicalturk.amazonaws.com/?';
	...
	*/	
	public function approveHIT($assignment_id) {
		$ts = $this->Unix2UTC(time());
	...
	public function grantBonus($worker_id, $assignment_id, $amount) {
		...
		$url = $this->startUrl();
		$url .= '&Operation=GrantBonus';
		$url .= '&Signature=' . $this->generateSignature($this->MTURK_SERVICE, 'GrantBonus', $ts);
		$url .= '&Timestamp=' . $ts;
		$url .= '&WorkerId='.$worker_id;
		$url .= '&AssignmentId=' . $assignment_id;
	...
	*/
	...
	public function getAssignmentID($hit_id) {
      $xml = $this->getAssignmentsForID($hit);
	...
	public function assignQualification($qtid, $worker_id, $value) {
		...
		$url = $this->startUrl();
		$url .= '&Operation=AssignQualification';
		$url .= '&Signature=' . $this->generateSignature($this->MTURK_SERVICE, 'AssignQualification', $ts);
		$url .= '&Timestamp=' . $ts;
		$url .= '&QualificationTypeId=' .$qtid;
		$url .= '&WorkerId=' . $worker_id;
		$url .= '&IntegerValue='.$value;
